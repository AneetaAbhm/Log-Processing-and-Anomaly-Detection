...
from log_processor import parser_func, process_logs
import tempfile
from pathlib import Path
import os

# TESTS FOR parser_func

def test_parser_func_valid():
...
def test_parser_func_malformed_wrong_parts():
    line = "2026-03-18 10:15:23 | ERROR | service_b"   # only 3 parts
    assert parser_func(line) is None

def test_parser_func_empty():
    assert parser_func("") is None
    assert parser_func("   ") is None

def test_parser_func_bad_timestamp():
    line = "2026-99-99 10:15:23 | INFO | service_a | hello"
    assert parser_func(line) is None

def test_parser_func_extra_spaces():
